    ...
    <div class="container">
        @component('layout.partials.components.dataTable', [
            ...
            <table class="table table-striped table-hover order-column" id="doctors">
                ...
                <tbody>
                @foreach(\App\Role::Doctors()->user()->withoutBanned()->get() as $user)
                    <tr class="odd gradeX">
                        <td>{{ $user->getKey() }}</td>
                        <td>
                        ...
                        <td>
                            @if ($user->specialities->count())
                                <span
                                    class="label label-primary label-sm">{{ $user->specialities->first()->display_name }}</span>

                                @if ($user->specialities->count() > 1)
                                    @php
                                        $specialities = $user->specialities->map(function($specialty) {
                                            return $specialty->display_name;
                                        })->all();
                                    @endphp
                                    <span class="label label-success pull-right tooltips" data-html="true"
                                          data-original-title="{!! implode('<br />', $specialities) !!}"
                                          data-container="body"
                                          data-placement="right">+ {{ $user->specialities->count() - 1 }}</span>
                                @endif
                            @endif
                        </td>
                        <td>
                        ...
                        <td class="text-right">
                            <div class="btn-group btn-group-xs">
                                ...
                                <a href="{{ route('admin.doctors.edit', $user) }}" class="btn btn-warning tooltips"
        ...
        @if (\App\Role::Doctors()->user()->onlyBanned()->count())
            @component('layout.partials.components.dataTable', [
                ...
                <table class="table table-striped table-hover order-column" id="banned_doctors">
                    ...
                    <tbody>
                    @foreach(\App\Role::Doctors()->user()->onlyBanned()->get() as $user)
                        <tr class="odd gradeX">
                            <td>{{ $user->getKey() }}</td>
                            <td>
                                <div class="label label-primary pull-left" style="margin:0 10px;">
        ...
        @foreach(\App\Role::Doctors()->user()->get() as $user)
            <div id="delete_user_{{ $user->getKey() }}" class="modal fade" data-keyboard="false" tabindex="-1"
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        {!! Form::open(['method' => 'DELETE', 'route' => ['admin.doctors.destroy', $user]]) !!}
            ...
            @if ($user->isNotBanned())
                <div id="ban_user_{{ $user->getKey() }}" class="modal fade" tabindex="-1" role="dialog"
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            {!! Form::open(['class' => 'form-horizontal ban-form', 'method' => 'patch', 'route' => ['admin.doctors.ban', $user]]) !!}
                            <div class="modal-header">
            ...
            @else
                <div id="unban_user_{{ $user->getKey() }}" class="modal fade" tabindex="-1" role="dialog"
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            {!! Form::open(['class' => 'form-horizontal ban-form', 'method' => 'patch', 'route' => ['admin.doctors.unban', $user]]) !!}
                            <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal"
                                        aria-hidden="true"></button>
